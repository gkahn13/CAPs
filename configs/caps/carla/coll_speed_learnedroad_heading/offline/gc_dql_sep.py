exp_name = 'caps/carla/coll_speed_learnedroad_heading/offline/gc_dql_sep'
seed = 0
log_level = 'debug'
...
def coll_bhat_label_func(rewards, dones, goals, target_obs_vec, gamma,
                         future_goals, target_yhats, target_bhats, target_values, tf_debug=None):
    import numpy as np
...
def speed_bhat_label_func(rewards, dones, goals, target_obs_vec, gamma,
                          future_goals, target_yhats, target_bhats, target_values):
    import numpy as np
    H = rewards.get_shape()[1].value
    gammas = np.power(gamma, np.arange(1, H+1))
    bhat_label = (1. - dones[:, 1:]) * gammas * (-1.0 *  tf.square((goals['speed'] - target_obs_vec['speed'])))
    return bhat_label

def rightlane_bhat_label_func(rewards, dones, goals, target_obs_vec, gamma,
                             future_goals, target_yhats, target_bhats, target_values):
    import numpy as np
    H = rewards.get_shape()[1].value
    gammas = np.power(gamma, np.arange(1, H+1))
    bhat_label = goals['rightlane_seen'] * (1 - abs(goals['rightlane_diff'])) + \
                 (1. - dones[:, 1:]) * gammas * target_values['rightlane']
    return bhat_label

def heading_bhat_label_func(rewards, dones, goals, target_obs_vec, gamma,
                             future_goals, target_yhats, target_bhats, target_values):
    import numpy as np
    H = rewards.get_shape()[1].value
    gammas = np.power(gamma, np.arange(1, H+1))
    ...
    heading = (np.pi / 180.) * target_obs_vec['heading']
    v = tf.concat([tf.clip_by_value(tf.cos(heading), -1.0 + 1e-4, 1.0 - 1e-4),
                   tf.clip_by_value(tf.sin(heading), -1.0 + 1e-4, 1.0 - 1e-4)], axis=1) # [batch_size, 2]
    v = tf.nn.l2_normalize(v, axis=1) # [batch_size, 2]

    heading_deviation = tf.acos(tf.clip_by_value(tf.reduce_sum(u * v, axis=1), -1.0 + 1e-4, 1.0 - 1e-4)) # [batch_size, H]
    heading_deviation /= np.pi # to put between [-1, 1]
    value = -heading_deviation[:, None]

    # deal with case in which ||heading_x, heading_y|| < 1e-2
    is_close = tf.cast(u_norm < 1e-2, tf.float32)[:, None]
    value = (1 - is_close) * value

    bhat_label = value + (1. - dones[:, 1:]) * gammas * target_values['heading']
...
policy_params = {
    ...
    'kwargs': {
        ...
        'action_selection_value': lambda actions, yhats, bhats, values, goals: \
                                    100 * (1-values['coll']) + 1 * values['speed'] + \
                                    5 * values['rightlane'] + \
                                    5 * values['heading'],
        ...
        'outputs': [
            {
                ...
                'bhat_label': coll_bhat_label_func,
                ...
                'bhat_loss_xentropy_posweight': 1.0,  # larger value --> false negatives cost more
            ...
            },
            {
                ...
                # :param target_bhats: {name: [batch_size, N, H]}
                # :param target_values: {name: [batch_size, N]}
                'bhat_label': speed_bhat_label_func,
                ...
                # :param goals: {name: [batch_size, 1]}
                # :param gamma
                'value': lambda yhats, bhats, goals, gamma: tf.reduce_mean(bhats['speed'], axis=1),
            ...
            },
            {
                ...
                # :param target_bhats: {name: [batch_size, N, H]}
                # :param target_values: {name: [batch_size, N]}
                'bhat_label': rightlane_bhat_label_func,
                ...
                # :param goals: {name: [batch_size, 1]}
                # :param gamma
                'value': lambda yhats, bhats, goals, gamma: tf.reduce_mean(bhats['rightlane'], axis=1),
            ...
            },
            {
                ...
                # :param target_bhats: {name: [batch_size, N, H]}
                # :param target_values: {name: [batch_size, N]}
                'bhat_label': heading_bhat_label_func,
                ...
                # :param goals: {name: [batch_size, 1]}
                # :param gamma
                'value': lambda yhats, bhats, goals, gamma: tf.reduce_mean(bhats['heading'], axis=1),
