exp_name = 'caps/carla/coll_speed_learnedroad_heading/offline/caps'
seed = 0
log_level = 'debug'
...
alg_params = {
    ...
    'kwargs': {
        ...
        'total_steps': 1.e+6,  # corresponding to number of env.step(...) calls
...
def heading_value_func(yhats, bhats, goals, gamma):
    # :param yhats: {name: [batch_size, H]}
    # :param bhats: {name: [batch_size, H]}
    # :param goals: {name: [batch_size, 1]}
    # :param gamma
    H = yhats['heading_cos'].get_shape()[1].value

    u = tf.concat([goals['heading_x'], goals['heading_y']], axis=1) # [batch_size, 2]
    u_norm = tf.norm(u, axis=1)
    u = tf.nn.l2_normalize(u, axis=1) # [batch_size, 2]
    u = tf.tile(tf.expand_dims(u, axis=1), (1, H, 1)) # [batch_size, H ,2]

    v = tf.stack([tf.clip_by_value(yhats['heading_cos'], -1.0 + 1e-4, 1.0 - 1e-4),
                  tf.clip_by_value(yhats['heading_sin'], -1.0 + 1e-4, 1.0 - 1e-4)], axis=2) # [batch_size, H, 2]
    v = tf.nn.l2_normalize(v, axis=2) # [batch_size, H, 2]

    heading_deviation = tf.acos(tf.clip_by_value(tf.reduce_sum(u * v, axis=2), -1.0 + 1e-4, 1.0 - 1e-4))  # [batch_size, H]
    heading_deviation /= np.pi # to put between [-1, 1]
    value = -tf.reduce_mean(tf.abs(heading_deviation), axis=1)

    # deal with case in which ||heading_x, heading_y|| < 1e-2
    is_close = tf.cast(u_norm < 1e-2, tf.float32)
    value = (1 - is_close) * value
...
policy_params = {
    ...
    'kwargs': {
        ...
        'goals_to_input': [], # which goals to input?
        ...
        'action_selection_value': lambda actions, yhats, bhats, values, goals: \
            ...
            3.0 * values['speed'] + \
            5.0 * tf.reduce_mean(
                yhats['rightlane_seen'][:, :] * \
                (1 - tf.abs(yhats['rightlane_diff'][:, :])),
                axis=1, keepdims=True) + \
            0.15 * tf.reduce_mean(-tf.square(actions['steer']), axis=1, keepdims=True) + \
            5.0 * values['heading_cos'],
        ...
        'outputs': [
            {
                ...
                'yhat_loss_xentropy_posweight': 1,  # larger value --> false negatives cost more
                ...
                # :param goals: {name: [batch_size, 1]}
                # :param gamma
                'value': lambda yhats, bhats, goals, gamma: tf.reduce_mean(1 - yhats['coll'], axis=1),
            ...
            },
            {
                ...
                # :param pre_yhat: [batch_size, H]
                # :param obs_vec: {name : [batch_size, 1]}
                'yhat': lambda pre_yhat, obs_vec: pre_yhat,
                ...
                # :param target_obs_vec: [batch_size, N]
                # :param gamma: scalar
                'yhat_label': lambda rewards, dones, goals, future_goals, target_obs_vec, gamma: target_obs_vec['speed'],

                # yhat training cost
                'yhat_loss': 'mse', # <mse / huber / xentropy>
                'yhat_loss_weight': 1.0, # how much to weight this loss compared to other losses
                'yhat_loss_use_pre': False,  # use the pre-activation for the loss? needed for xentropy
                ...
                # :param goals: {name: [batch_size, 1]}
                # :param gamma
                'value': lambda yhats, bhats, goals, gamma: \
                    tf.reduce_mean(-1 * tf.abs((yhats['speed'] - goals['speed']) / goals['speed']), axis=1),
            ...
            },
            {
                ...
                # :param gamma: scalar
                'yhat_label': lambda rewards, dones, goals, future_goals, target_obs_vec, gamma: \
                    future_goals['rightlane_seen'],

                # yhat training cost
                'yhat_loss': 'xentropy',  # <mse / huber / xentropy>
                'yhat_loss_weight': 1.0,  # how much to weight this loss compared to other losses
                'yhat_loss_use_pre': True,  # use the pre-activation for the loss? needed for xentropy
                'yhat_loss_xentropy_posweight': 1.0,  # larger value --> false negatives cost more
                ...
                # :param goals: {name: [batch_size, 1]}
                # :param gamma
                'value': lambda yhats, bhats, goals, gamma: np.nan * tf.ones(1),
            ...
            },
            {
                ...
                # :param pre_yhat: [batch_size, H]
                # :param obs_vec: {name : [batch_size, 1]}
                'yhat': lambda pre_yhat, obs_vec: tf.nn.tanh(pre_yhat),
            ...
            },
            {
                ...
                # :param pre_yhat: [batch_size, H]
                # :param obs_vec: {name : [batch_size, 1]}
                'yhat': lambda pre_yhat, obs_vec: 2.0 * tf.tanh(pre_yhat) + tf.cos((np.pi / 180.) * obs_vec['heading']),
                ...
                # :param gamma: scalar
                'yhat_label': lambda rewards, dones, goals, future_goals, target_obs_vec, gamma: \
                    tf.cos((np.pi / 180.) * target_obs_vec['heading']),

                # yhat training cost
                'yhat_loss': 'mse', # <mse / huber / xentropy>
                'yhat_loss_weight':1., # how much to weight this loss compared to other losses
                'yhat_loss_use_pre': False,  # use the pre-activation for the loss? needed for xentropy
                'yhat_loss_xentropy_posweight': None,  # larger value --> false negatives cost more
            ...
            },
            {
                ...
                # :param pre_yhat: [batch_size, H]
                # :param obs_vec: {name : [batch_size, 1]}
                'yhat': lambda pre_yhat, obs_vec: 2.0 * tf.tanh(pre_yhat) + tf.sin((np.pi / 180.) * obs_vec['heading']),
                ...
                # :param gamma: scalar
                'yhat_label': lambda rewards, dones, goals, future_goals, target_obs_vec, gamma: \
                    tf.sin((np.pi / 180.) * target_obs_vec['heading']),
